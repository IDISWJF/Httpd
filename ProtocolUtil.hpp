...
class Entry{
	public:
		...
		static int HandlerRequest(struct bufferevent* bev)//处理请求
		{
			...
			//获取libevent buf中数据的长度
			struct evbuffer *input = bufferevent_get_input(bev);
			int recv_size = (int)evbuffer_get_length(input);
			//返回第一缓冲链中的连续可用字节数
			//这在处理可能拆分为多个数据时很有用
...
Cache-Control: max-age=0
			 */
			/////////////////////////////////////////////////////////////////////////////////////
			//将socket的调用的方式封装为对bev事件的处理，
			//可通过出差分缓冲区字符串对报头进行解析
			//对之后输出请忽略
			LOG(INFO,"start recv request");
			conn_-> RecvOneLine(rq_->rq_line);//接收请求行
			std::cout<<"recv_line："<<rq_->rq_line<<std::endl;
